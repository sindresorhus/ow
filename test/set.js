...
test('set.ofType', t => {
	...
	t.throws(() => {
		...
		message: '(Set) Expected argument to be of type `number` but received type `string`'
	});
	t.throws(() => {
		ow(new Set(['unicorn']), 'foo', ow.set.ofType(ow.number));
	}, {
		message: '(Set `foo`) Expected argument to be of type `number` but received type `string`'
	});
});
test('set.empty', t => {
	t.notThrows(() => {
