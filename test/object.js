...
test('object.valuesOfType', t => {
	...
	t.throws(() => {
		...
		message: '(object) Expected argument to be of type `string` but received type `number`'
	});

	t.throws(() => {
		ow({unicorn: '🦄', rainbow: 2}, 'foo', ow.object.valuesOfType(ow.string));
	}, {
		message: '(object `foo`) Expected argument to be of type `string` but received type `number`'
	});

	t.throws(() => {
		ow({unicorn: 'a', rainbow: 'b'}, ow.object.valuesOfType(ow.string.minLength(2)));
	}, {
		message: '(object) Expected string to have a minimum length of `2`, got `a`'
	});
...
test('object.valuesOfTypeDeep', t => {
	...
	t.throws(() => {
		...
		message: '(object) Expected argument to be of type `number` but received type `string`'
	});
});
