...
test('map.keysOfType', t => {
	...
	t.throws(() => {
		...
		message: '(Map) Expected argument to be of type `number` but received type `string`'
	});

	t.throws(() => {
		ow(new Map([['unicorn', '🦄']]), 'foo', ow.map.keysOfType(ow.number));
	}, {
		message: '(Map `foo`) Expected argument to be of type `number` but received type `string`'
	});
});
