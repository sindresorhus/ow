...
import ow, {ArgumentError} from '../source/index.js';
import {createAnyError} from './fixtures/create-error.js';
import isNode from '../source/utils/node/is-node.js';
...
test('any-reusable validator', t => {
	...
	t.throws(() => {
		checkUsername('fo');
	}, {message: createAnyError('Expected string to include `.`, got `fo`', 'Expected string to have a minimum length of `3`, got `fo`')});
	t.throws(() => {
		checkUsername(5);
	}, {message: createAnyError('Expected argument to be of type `string` but received type `number`', 'Expected string to include `.`, got `5`', 'Expected string to have a minimum length of `3`, got `5`')});
});
...
// the test below is probably obsolete, but it's commented out because I don't know how to deal with this without using `require`
...
// 	});
// 	Object.defineProperty(isNode, 'default', {
// 		value: false
// 	});
// 	t.throws(() => {
// 		ow('owo', ow.string.equals('OwO'));
